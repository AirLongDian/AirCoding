...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, AirCoding does not support MCP authentication yet.`
  }
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      `Try: \`aircoding models\` to list available models`,
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`aircoding auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
