...
const AIRRVR_SPEC: Record<string, { name: string; task: string; tools: string; execRequired: boolean }> = {
  ...
  "R-12": { name: "高风险操作异常回退机制", task: "识别关键高风险操作（文件写入/网络请求/DB 事务/配置变更/状态机转换），逐处确认有异常回退或恢复机制（事务回滚/临时文件+原子替换/重试+补偿逻辑），并确认回退路径真实可达", tools: "grep + read", execRequired: false },
...
function buildAirRvrWorkerRetryPrompt(task: TaskGraphTask, missing: string[]): string {
  ...
  const spec = [
    ...
    ["R-12", "高风险操作异常回退机制", "文件写入/网络/DB 事务/配置变更/状态机转换须有回退或恢复机制"],
...
function buildReviewPrompt(task: TaskGraphTask, workerResult: string): string {
  const items = [
    ...
    ["R-12", "高风险操作异常回退机制", "关键高风险操作须有异常回退或恢复机制，回退路径真实可达"],
